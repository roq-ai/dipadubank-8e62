...
import { getCryptoTransactions } from 'apiSdk/crypto-transactions';
...
import { getCryptoWallets } from 'apiSdk/crypto-wallets';
...
function DashboardPage() {
  ...
  const cryptoTransactionStat = useSWR<PaginatedInterface<CryptoTransactionInterface>>(
    () => `/crypto-transactions`,
    () => getCryptoTransactions({ limit: 1, offset: 0 }),
  );
  ...
  const cryptoWalletStat = useSWR<PaginatedInterface<CryptoWalletInterface>>(
    () => `/crypto-wallets`,
    () => getCryptoWallets({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Crypto Transactions',
      result: cryptoTransactionStat?.data?.totalCount,
      isLoading: cryptoTransactionStat.isLoading,
    ...
    },
    {
      ...
      result: cryptoWalletStat?.data?.totalCount,
      isLoading: cryptoWalletStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
